    ...
    def _create_test_payment_link(self, case: dict) -> dict:
        if not settings.ENABLE_RAZORPAY_TEST_ACTIONS:
            return {
                'mode': 'SIMULATION',
                'action': 'PAYMENT_LINK',
                'execution_boundary': 'Razorpay Test Mode disabled; no external call made',
            }
        ...
        payload = {
            ...
            'reference_id': case.get('case_id', case.get('payment_id', 'recoverai')),
        }
        customer = {}
        ...
        if case.get('customer_contact'):
            customer['contact'] = case['customer_contact']
        if customer:
        ...
        try:
            ...
            return {
                ...
                'execution_boundary': 'Razorpay Test Mode only',
            }
        except (urllib.error.URLError, urllib.error.HTTPError, json.JSONDecodeError) as exc:
            return {
                'mode': 'RAZORPAY_TEST_MODE',
                'action': 'PAYMENT_LINK',
                'execution_boundary': 'External action failed safely',
                'error': str(exc),
            }

    def execute(self, db, case: dict, policy_decision: str, action: str, policy_decision_id: str = ''):
        allowed_decisions = {'allow', 'human_review'}
        if policy_decision not in allowed_decisions:
        ...
        if policy_decision == 'human_review' or action == ActionType.HUMAN_ESCALATION.value:
            ...
            details = {
                ...
                'action': action,
                'reason': 'Policy requires compliant human escalation',
            }
            ...
            details = {
                ...
                'recovered_amount': float(case.get('amount', 0) or 0),

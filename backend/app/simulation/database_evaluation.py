...
def evaluate_database_payments(db, limit: int = 1000, batch_id: str | None = None):
    ...
    if dataset_rows and ml_model.training_rows == 0:
    ...
    for row in rows:
        ...
        data['ml_recoverability'] = ml_prediction['recoverability_probability']; data['ml_confidence'] = ml_prediction['confidence']
        ...
        policy = policy_engine.evaluate(case); decision = policy['decision']; counts[decision] = counts.get(decision, 0) + 1; action_counts[action] = action_counts.get(action, 0) + 1

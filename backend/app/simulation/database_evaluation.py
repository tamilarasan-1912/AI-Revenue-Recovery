...
def evaluate_database_payments(db, limit: int = 1000, batch_id: str | None = None):
    ...
    dataset_rows = [{'payment_id': r.payment_id, 'amount': r.amount, 'failure_reason': r.failure_reason, 'retry_count': r.retry_count, 'is_recoverable': r.is_recoverable} for r in rows]
    ...
    for row, ml_prediction in zip(rows, predictions):
        data = {'payment_id': row.payment_id, 'amount': float(row.amount or 0), 'failure_reason': row.failure_reason or 'unknown', 'retry_count': int(row.retry_count or 0), 'ml_recoverability': ml_prediction['recoverability_probability'], 'ml_confidence': ml_prediction['confidence']}
        ...
        if row.is_recoverable: recoverable_revenue += data['amount']
        ...
        if recovered: recovered_revenue += data['amount']; recovered_records += 1

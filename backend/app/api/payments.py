...
def list_payments(limit: int = Query(50, ge=1, le=200), db: Session = Depends(get_db)):
    source_filter = Payment.payment_method.like('csv_demo:%')
    ...
    return {'summary': summary, **summary, 'payments': [{'payment_id': p.id, 'amount': p.amount, 'status': p.status.value if p.status else None, 'payment_method': p.payment_method, 'failure_reason': p.failure_reason, 'retry_count': p.retry_count, 'created_at': p.created_at} for p in rows]}

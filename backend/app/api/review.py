...
def create_demo_review_case(scenario: str | None = Query(None), db: Session = Depends(get_db)):
    ...
    action = scenario_data['action'].value
    retry_count = scenario_data['retry_count']

    payment = Payment(id=payment_id, amount=amount, status=PaymentStatus.FAILED, payment_method='card', failure_reason=scenario_data['failure_reason'], retry_count=retry_count)
    case = RecoveryCase(id=case_id, payment_id=payment_id, revenue_at_risk=amount, recommended_action=scenario_data['action'], ai_confidence=confidence)
    policy_result = policy_engine.evaluate({'case_id': case_id, 'payment_id': payment_id, 'amount': amount, 'recommended_action': action, 'ai_confidence': confidence, 'retry_count': retry_count, 'expected_recovery_value': scenario_data['expected_recovery_value'], 'fraud_signal': scenario_data['fraud_signal']})
    ...
    if decision == 'human_review': execution_status = 'PENDING_HUMAN_REVIEW'
    elif decision in {'stop', 'block'}: execution_status = 'STOPPED' if decision == 'stop' else 'BLOCKED'
    else: execution_status = 'PENDING'

    attempt = retry_count if action == ActionType.RETRY.value else 0
    idempotency_key = f'recover:{case_id}:{action}:{attempt}'
    execution = ExecutionRecord(id=execution_id, policy_decision_id=policy_id, action=scenario_data['action'], status=execution_status, idempotency_key=idempotency_key, result_details={'demo': True, 'scenario': scenario_data['label'], 'ai_confidence': confidence, 'policy_decision': decision, 'policy_rules': policy_result['rules_triggered'], 'note': 'Synthetic case; no real-money movement'})
    db.add(execution)
    ...
    return {'status': execution_status, 'payment_id': payment_id, 'case_id': case_id, 'execution_id': execution_id, 'amount': amount, 'failure_reason': scenario_data['failure_reason'], 'recommended_action': action, 'ai_confidence': confidence, 'scenario': scenario_data['label'], 'policy_decision': decision, 'policy_rules': policy_result['rules_triggered'], 'policy_version': policy_result['policy_version'], 'requires_human_review': decision == 'human_review', 'can_execute': decision == 'allow', 'demo': True}
...
def decide_review(execution_id: str, approved: bool, reviewer: str = 'merchant_reviewer', db: Session = Depends(get_db)):
    row, policy, case, payment = _load_execution_case(db, execution_id)
    if row.status != 'PENDING_HUMAN_REVIEW': raise HTTPException(status_code=409, detail='Review item is no longer pending')
    ...
    if not approved:
        row.status = 'REJECTED_BY_HUMAN'; row.result_details = {**(row.result_details or {}), 'reason': 'Merchant rejected recovery action'}
        db.add(AuditLog(id=f'audit_{uuid.uuid4().hex}', event_id=f'review_{row.id}', payment_id=payment.id, action=row.action.value, outcome='REJECTED_BY_HUMAN')); db.commit()

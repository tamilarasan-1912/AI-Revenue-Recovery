...
DEMO_SCENARIOS = [
    {
        'amount': 7499.0, 'confidence': 0.94,
        'failure_reason': 'Synthetic demo: bank timeout; immediate retry is appropriate',
        'action': ActionType.RETRY, 'retry_count': 0, 'expected_recovery_value': 5999.20,
        'fraud_signal': False, 'label': 'BANK_TIMEOUT_ALLOW',
    },
    {
        'amount': 2899.0, 'confidence': 0.90,
        'failure_reason': 'Synthetic demo: insufficient funds; payment link is preferred',
        'action': ActionType.PAYMENT_LINK, 'retry_count': 0, 'expected_recovery_value': 2319.20,
        'fraud_signal': False, 'label': 'INSUFFICIENT_FUNDS_PAYMENT_LINK',
    },
    {
        'amount': 19999.0, 'confidence': 0.97,
        'failure_reason': 'Synthetic demo: fraud signal detected; recovery must stop',
        'action': ActionType.STOP, 'retry_count': 0, 'expected_recovery_value': 0.0,
        'fraud_signal': True, 'label': 'FRAUD_STOP',
    },
    {
        'amount': 1499.0, 'confidence': 0.95,
        'failure_reason': 'Synthetic demo: retry budget exhausted',
        'action': ActionType.RETRY, 'retry_count': 3, 'expected_recovery_value': 1199.20,
        'fraud_signal': False, 'label': 'RETRY_EXHAUSTION_STOP',
    },
    {
        'amount': 12500.0, 'confidence': 0.42,
        'failure_reason': 'Synthetic demo: insufficient evidence for automatic recovery',
        'action': ActionType.RETRY, 'retry_count': 0, 'expected_recovery_value': 10000.0,
        'fraud_signal': False, 'label': 'LOW_CONFIDENCE_HUMAN_REVIEW',
    },
]
...
def create_demo_review_case(db: Session = Depends(get_db)):
    ...
    payment = Payment(id=payment_id, amount=amount, status=PaymentStatus.FAILED, payment_method='card', failure_reason=scenario['failure_reason'], retry_count=retry_count)
    case = RecoveryCase(id=case_id, payment_id=payment_id, revenue_at_risk=amount, recommended_action=scenario['action'], ai_confidence=confidence)
    policy_result = policy_engine.evaluate({
        'case_id': case_id, 'payment_id': payment_id, 'amount': amount,
        'recommended_action': action, 'ai_confidence': confidence,
        'retry_count': retry_count, 'expected_recovery_value': scenario['expected_recovery_value'],
        'fraud_signal': scenario['fraud_signal'],
    })
    policy = PolicyDecisionRecord(id=policy_id, recovery_case_id=case_id, decision=policy_result['decision'], policy_version=policy_result['policy_version'], rules_triggered=policy_result['rules_triggered'])
    db.add_all([payment, case, policy]); db.flush()
    ...
    # Match the executor's canonical key so clicking Execute updates this same record.
    attempt = retry_count if action == ActionType.RETRY.value else 0
    idempotency_key = f'recover:{case_id}:{action}:{attempt}'
    execution = ExecutionRecord(
        id=execution_id, policy_decision_id=policy_id, action=scenario['action'], status=execution_status,
        idempotency_key=idempotency_key,
        result_details={'demo': True, 'scenario': scenario['label'], 'ai_confidence': confidence, 'policy_decision': decision, 'policy_rules': policy_result['rules_triggered'], 'note': 'Synthetic case; no real-money movement'},
    )
    db.add(execution)
    db.add(AuditLog(id=f'audit_{uuid.uuid4().hex}', event_id=f'demo_created_{execution_id}', payment_id=payment_id, action=action, outcome=execution_status))
    db.commit()

    return {
        'status': execution_status, 'payment_id': payment_id, 'case_id': case_id, 'execution_id': execution_id,
        'amount': amount, 'failure_reason': scenario['failure_reason'], 'recommended_action': action,
        'ai_confidence': confidence, 'scenario': scenario['label'], 'policy_decision': decision,
        'policy_rules': policy_result['rules_triggered'], 'policy_version': policy_result['policy_version'],
        'requires_human_review': decision == 'human_review', 'can_execute': decision == 'allow', 'demo': True,
    }
